...
<div class="position-absolute pb-5">
    <a href="/jadwalxirplb" class="btn btn-secondary top-0 ms-5">
        <i class="fa-solid fa-arrow-left"></i> Kembali
    </a>
</div>

<div style="background-color: #f3f1f1;">
    <form  class="text-start mx-auto my-5 bg-white card" method="POST" action="{{ route('jadwal_rplb.update', $jadwal_rplb->id) }}">
        @csrf
        @method('PUT')
        <div class="mb-3">
            <h2 class="fw-bold text-center">Edit Data Jadwal XI RPL B</h2> <br>
            ...
            <select class="form-select" name="hari" aria-label="Default select example">
                <option value="{{ $jadwal_rplb->hari }}" selected>{{ $jadwal_rplb->hari }}</option>
        ...
        <div class="mb-3">
            ...
            <select name="siswa_1" id="siswa1a" class="form-select">
                <option value="{{ $jadwal_rplb->siswa_1 }}" selected>{{ $jadwal_rplb->siswa_1 }}</option>
                @foreach ($b as $sb)
                    <option value="{{ $sb->nama_siswa }}">{{ $sb->nama_siswa }}</option>
                @endforeach
            </select>
        </div>
        <div class="mb-3">
            ...
            <select name="siswa_2" id="siswa2a" class="form-select">
                <option value="{{ $jadwal_rplb->siswa_2 }}" selected>{{ $jadwal_rplb->siswa_2 }}</option>
                @foreach ($b as $sb)
                    <option value="{{ $sb->nama_siswa }}">{{ $sb->nama_siswa }}</option>
                @endforeach
            </select>
        ...
        <div class="mb-3">
            ...
            <select name="siswa_3" id="siswa3a" class="form-select">
                <option value="{{ $jadwal_rplb->siswa_3 }}" selected>{{ $jadwal_rplb->siswa_3 }}</option>
                @foreach ($b as $sb)
                    <option value="{{ $sb->nama_siswa }}">{{ $sb->nama_siswa }}</option>
                @endforeach
            </select>
        ...
        <div class="mb-3">
            ...
            <select name="siswa_4" id="siswa4a" class="form-select">
                <option value="{{ $jadwal_rplb->siswa_4 }}" selected>{{ $jadwal_rplb->siswa_4 }}</option>
                @foreach ($b as $sb)
                    <option value="{{ $sb->nama_siswa }}">{{ $sb->nama_siswa }}</option>
                @endforeach
            </select>
        ...
        <div class="mb-3">
            ...
            <select name="siswa_5" id="siswa5a" class="form-select">
                <option value="{{ $jadwal_rplb->siswa_5 }}" selected>{{ $jadwal_rplb->siswa_5 }}</option>
                @foreach ($b as $sb)
                    <option value="{{ $sb->nama_siswa }}">{{ $sb->nama_siswa }}</option>
                @endforeach
            </select>
