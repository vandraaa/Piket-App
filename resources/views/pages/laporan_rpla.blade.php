...
@section('content')
    ...
    <div class="isi-content">
        <a href="/" class="text-black text-decoration-none"><i class="fa-solid fa-house"></i> >
            ...
            aria-expanded="false" aria-controls="jadwalContent" class="text-black text-decoration-none">Laporan > XI RPL A</a>
        </a>
        <h2 class="text-center text-3xl fw-bold">Laporan Jadwal Piket RPL A</h2>
        <div class="overflow-x-auto">
            <table class="table table-bordered table-striped mx-auto mt-4" style="width: max-content;">
                <thead>
                    <tr class="text-center align-middle px-5">
                        <th>Tanggal</th>
                        <th>Hari</th>
                        <th>Siswa 1</th>
                        <th>Status</th>
                ...
                <tbody>
                    @foreach ($laporanA as $la)
                        <tr class="text-center align-middle" style="font-size: 13px;">
                            ...
                            @if ($user->role != 'ADMIN B' && $user->role != 'ADMIN C' && $user->role != 'GUEST')
                                <td>
                                    <a href="{{ route('laporan_rpla.edit', $la->id) }}">
        ...
        <div class="d-flex justify-content-between align-items-center mt-2">
            <div>
                <a href="{{ route('laporan-rpla.download-excel') }}" class="btn btn-primary text-start">Download
                    Laporan</a>
            </div>
            @if ($user->role != 'ADMIN B' && $user->role != 'ADMIN C' && $user->role != 'GUEST')
                <button class="btn btn-danger" onclick="confirmClearA()">Clear Data</button>
            @endif
        </div>
    </div>
    ...
    <div class="modal" id="confirmrpla" style="display: none;">
        <div class="notif-success">
            ...
            <form id="clearForm" action="{{ route('laporan_rpla.clear') }}" method="post">
                @csrf
