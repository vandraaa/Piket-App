...
<div class="isi-content">
    ...
    <h2 class="text-center text-3xl fw-bold">Laporan Jadwal Piket RPL B</h2>
    <div class="overflow-x-auto">
        <table class="table table-bordered table-striped mx-auto mt-4" style="width: max-content;">
            <thead>
                <tr class="text-center">
                    ...
                    <th>Siswa 5</th>
                    <th>Status</th>
                    @if ($user->role != 'ADMIN C' && $user->role != 'ADMIN A' && $user->role != 'GUEST')
                        <th>Edit</th>
                    @endif
                </tr>
            </thead>
            <tbody>
                @foreach ($laporanB as $la)
                    <tr class="text-center align-middle" style="font-size: 13px;">
                        ...
                        @if ($user->role != 'ADMIN C' && $user->role != 'ADMIN A' && $user->role != 'GUEST')
                            <td>
                                <a href="{{ route('laporan_rplb.edit', $la->id) }}">
        ...
        <div class="d-flex justify-content-between align-items-center mt-2">
            <div>
                ...
                <a href="{{ route('laporan-rplb.download-excel') }}"
                    class="btn btn-primary text-start">Download
                    Laporan</a>
            </div>
            @if ($user->role != 'ADMIN C' && $user->role != 'ADMIN A' && $user->role != 'GUEST')
...
@if (session('showSuccessPopup'))
    ...
    <div class="modal" id="confirmrplb" style="display: none;">
        <div class="notif-success">
            ...
            <form id="clearForm" action="{{ route('laporan_rplb.clear') }}" method="post">
                @csrf
