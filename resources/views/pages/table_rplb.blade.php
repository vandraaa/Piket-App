...
@section('content')
    ...
    <div class="isi-content">
        <a href="/" class="text-black text-decoration-none"><i class="fa-solid fa-house"></i> >
            ...
            aria-expanded="false" aria-controls="jadwalContent" class="text-black text-decoration-none">Jadwal > XI RPL B</a>
        </a>
        <div class=" w-100 mx-auto" id="contentB">
            <h2 class="text-center text-3xl fw-bold">Kelas XI RPL B</h2>
            <div class="d-flex justify-content-between">
                @if ($user->role != 'ADMIN A' && $user->role != 'ADMIN C' && $user->role != 'GUEST')
                    <button onclick="openFormJadwalB()" class="btn btn-primary">+</button>
                @endif
            </div>
            <div class="overflow-x-auto">
                                ...
                                @if ($user->role != 'ADMIN A' && $user->role != 'ADMIN C' && $user->role != 'GUEST')
                                    <td>
                                        <div class="d-flex justify-content-center gap-2">
                                            <a href="{{ route('jadwal_rplb.edit', $rb->id) }}">
                                                <button class="btn btn-success"><i
                                                        class="fa-solid fa-pen-to-square"></i></button>
                                            </a>
                                            <form action="{{ route('jadwal.rplb.destroy', $rb->id) }}" method="POST"
                                                class="m-0">
                                                @csrf
                                                @method('DELETE')
                ...
                <div class="d-flex justify-content-between align-items-center">
                    <div>
                        <a href="{{ route('jadwal-rplb.download-excel') }}" class="btn btn-primary">Download Jadwal</a>
                    </div>
                    @if ($user->role != 'ADMIN C' && $user->role != 'GUEST' && $user->role != 'ADMIN A')
                        <button class="btn btn-danger" onclick="confirmClearB()">Clear Data</button>
                    @endif
                </div>
            </div>
        </div>
